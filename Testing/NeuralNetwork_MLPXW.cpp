...
int main(int argc, char const *argv[]) {
    ...
    // ======================= layer 1=======================
    Tensor   *_w1 = Tensor::Truncated_normal(1, 1, 1, 2, 2, 0.0, 0.6);
    ...
    Softmax_Cross_Entropy *err = new Softmax_Cross_Entropy(add_1, ans,"softmax_cross_entropy_1");
    ...
    for (int i = 0; i < atoi(argv[1]); i++) {
        ...
        x1->GetOutput()->PrintData();
        w1->GetOutput()->PrintData();
        mat_1->GetOutput()->PrintData();
        b1->GetOutput()->PrintData();
        add_1->GetOutput()->PrintData();
        err->GetSoftmaxResult()->PrintData();
        ans->GetOutput()->PrintData();
        err->GetOutput()->PrintData();
    ...
    for (int i = 0; i < 5; i++) {
        ...
        std::cout << "Accuracy is : " << Accuracy(add_1, ans) << '\n';
    }
