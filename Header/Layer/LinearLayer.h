...
private:
public:
    Linear(Operator<float> *pInput, int pNumInputCol, int pNumOutputCol, int use_bias = FALSE, std::string pName = NULL) {
    ...
    int Alloc(Operator<float> *pInput, int pNumInputCol, int pNumOutputCol, int use_bias, std::string pName) {
        ...
        Tensorholder<DTYPE> *pWeight = this->AddParameter(new Tensorholder<DTYPE>(Tensor<DTYPE>::Truncated_normal(1, 1, 1, pNumInputCol, pNumOutputCol, 0.0, 0.1), "Layer_Weight_" + pName));
