...
namespace layer {
template<typename DTYPE> class Convolution2D : public Layer<DTYPE>{
private:
public:
    Convolution2D(Operator<DTYPE> *pInput, int pNumInputChannel, int pNumOutputChannel, int pNumKernelRow, int pNumKernelCol, int pStrideRow, int pStrideCol, int pPadding = SAME, int use_bias = FALSE, std::string pName = "NO NAME") {
    ...
    int Alloc(Operator<DTYPE> *pInput, int pNumInputChannel, int pNumOutputChannel, int pNumKernelRow, int pNumKernelCol, int pStrideRow, int pStrideCol, int pPadding, int use_bias, std::string pName) {
        ...
        out = this->AddOperator(new op::Convolution2D<DTYPE>(out, pWeight, pStrideRow, pStrideCol, pPadding, "Convolution2D_Convolution2D_" + pName));
