#ifndef THRESHOLD_H_
#define THRESHOLD_H_    value

#include "..//Operator.h"

template<typename DTYPE>
class Threshold : public Operator<DTYPE>{
private:
...
public:
    ...
    // 나머지 MetaParameter에 대한 Alloc()을 진행한다. (Threshold::Alloc())
    Threshold(Operator<DTYPE> *pInput, std::string pName) : Operator<DTYPE>(pInput, pName) {
        std::cout << "Threshold::Threshold(Operator<DTYPE> *)" << '\n';
        this->Alloc(pInput);
    }
    ...
    virtual int Alloc(Operator<DTYPE> *pInput) {
        std::cout << "Threshold::Alloc(Operator<DTYPE> *, Operator<DTYPE> *)" << '\n';
                ...
                for (int ch = 0; ch < shape[2]; ch++) {
                    for (int ro = 0; ro < shape[3]; ro++) {
                        for (int co = 0; co < shape[4]; co++) {
                            if(input[ti][ba][ch][ro][co] > 0.0) output[ti][ba][ch][ro][co] = 8.8;
                            else output[ti][ba][ch][ro][co] = 0.0;
                        }
                    }
                }
            }
