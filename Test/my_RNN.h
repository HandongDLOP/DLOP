...
public:
    my_RNN(Tensorholder<float> *x, Tensorholder<float> *label) {
        ...
        Operator<float> *w1 = AddTensorholder(new Tensorholder<float>(Tensor<float>::Truncated_normal(1, 1, 1, 4, 2, 0.0, 0.1), "weight1"));
        Operator<float> *b1 = AddTensorholder(new Tensorholder<float>(Tensor<float>::Constants(1, 1, 1, 1, 2, 0.1), "bias1"));
        Operator<float> *w2 = AddTensorholder(new Tensorholder<float>(Tensor<float>::Truncated_normal(1, 1, 1, 2, 2, 0.0, 0.1), "weight2"));
        Operator<float> *b2 = AddTensorholder(new Tensorholder<float>(Tensor<float>::Constants(1, 1, 1, 1, 2, 0.1), "bias2"));
        Operator<float> *w3 = AddTensorholder(new Tensorholder<float>(Tensor<float>::Truncated_normal(1, 1, 1, 2, 2, 0.0, 0.1), "weight3"));
        Operator<float> *b3 = AddTensorholder(new Tensorholder<float>(Tensor<float>::Constants(1, 1, 1, 1, 2, 0.1), "bias3"));
